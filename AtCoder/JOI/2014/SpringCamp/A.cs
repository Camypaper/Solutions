    ...
    public class Solver
    {
        public void Solve()
        {
            ...
            for (int i = 0; i < m; i++)
            {
                ...
                var s = sc.Integer();
                var e = sc.Integer();
                G[b].Add(new Edge(b, a, e, s));
            }
            foreach (var g in G)
                g.Sort((l, r) => l.start.CompareTo(r.start));
            var Q = sc.Integer();
            var TL = sc.Integer(Q);
            var id = Enumerate(Q, x => x);
            Array.Sort(id, (l, r) => TL[l].CompareTo(TL[r]));
            var iter = new int[n];
            var start = new int[n];
            for (int i = 0; i < n; i++)
                start[i] = -1;
            ...
            {
                var when = TL[id[_]];
                var pq = new PriorityQueue<KeyValuePair<int, int>>((l, r) => r.Value.CompareTo(l.Value));
                start[n - 1] = when;
                pq.Enqueue(new KeyValuePair<int, int>(n - 1, when));
                while (pq.Any())
                {
                    var p = pq.Dequeue();
                    var pos = p.Key;
                    var st = p.Value;
                    while (iter[pos] < G[pos].Count)
                    {
                        var e = G[pos][iter[pos]];
                        Debug.WriteLine("{0}->{1} {2}->{3}",e.from,e.to,e.start,e.end);
                        if (e.start <= st)
                        {
                            if(e.end>start[e.to])
                            {
                                start[e.to] = e.end;
                                pq.Enqueue(new KeyValuePair<int, int>(e.to, e.end));
                            }
                            iter[pos]++;
                        }
                        else break;
...
struct Edge
{
    ...
    public Edge(int f, int t, int s, int e)
        : this()
    {
        from = f; to = t;
        start = s; end = e;
