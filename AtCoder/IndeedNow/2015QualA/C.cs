...
namespace Program
{
    public class Solver
    {
        ...
        {
            var n = sc.Integer();
            var a = sc.Long(n).Where(x => x != 0).ToArray();
            var zip = new Zipper(a);
            var b = new int[zip.values.Length];
            var c = new long[zip.values.Length];
            foreach (var x in a)
            {
                var p = zip.Position(x);
                b[p]++;
                c[p] = x;
            }
            Array.Reverse(b);
            ...
            var q = sc.Integer();
            foreach (var x in sc.Integer(q))
            {
                var ub = b.UpperBound(x);
                if (ub == b.Length)
                    IO.Printer.Out.WriteLine(0);
                else IO.Printer.Out.WriteLine(c[ub] + 1);
...
public class Zipper
{
    ...
    public KeyValuePair<int, int> UnZip(long v)
    {
        var i = Array.BinarySearch(values, v);
        if (i < 0) i = ~i;
        return new KeyValuePair<int, int>(i, idx[i]);
    }
    public int Position(long v)
    {
        var i = Array.BinarySearch(values, v);
        return i >= 0 ? i : ~i;
    }
    public int Index(long v)
    {
        var i = Array.BinarySearch(values, v);
        return i >= 0 ? idx[i] : idx[~i];
    }
}
#endregion
