        ...
        public void Solve()
        {
            ...
            for (int i = 0; i < m; i++)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    if (pass[j])
                        continue;
                    G.AddDirectedEdge(j, i + n, int.MaxValue);
                }
                G.AddDirectedEdge(i + n, t, c[i]);
            }
            var ans = Flow.GetMaxFlow(G, s, t);
            if (ans >= w)
                IO.Printer.Out.WriteLine("SHIROBAKO");
            else IO.Printer.Out.WriteLine("BANSAKUTSUKITA");
