...
namespace Program
{
    ...
    public class Solver
    {
        public void Solve()
        {
            ...
            var pq = new PriorityQueue<string>(StringComparer.Ordinal);
            foreach (var x in sc.Scan(n))
                pq.Enqueue(x + (char)('z' + 1));
            while (pq.Any())
            {
                var p = pq.Dequeue();
                sb.Append(p[0]);
                if (p.Length > 2)
                    pq.Enqueue(p.Substring(1));
            }
            IO.Printer.Out.WriteLine(sb.ToString());
