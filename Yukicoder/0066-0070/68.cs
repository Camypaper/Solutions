...
namespace Program
{
    public class Solver
    {
        public void Solve()
        {
            var n = sc.Integer();
            var a = sc.Integer(n);
            var table = new double[500050];
            var pq = new BinaryHeapPriorityQueue<otaku>((l, r) => r.d.CompareTo(l.d));
            foreach (var x in a)
                pq.Enqueue(new otaku(x, 1));
            var ptr = 0;
            while (pq.Any() && ptr < 500010)
            {
                var p = pq.Dequeue();
                ptr++;
                table[ptr] = p.d;
                pq.Enqueue(new otaku(p.len, p.cnt + 1));
            }
            var m = sc.Integer();
            var b = sc.Integer(m);
            for (int i = 0; i < m; i++)
                IO.Printer.Out.WriteLine(table[b[i]]);
        }
...
struct otaku
{
    public int len, cnt;
    public double d;
    public otaku(int l, int c) : this() { len = l; cnt = c; d = 1.0 * len / (cnt); }
}
