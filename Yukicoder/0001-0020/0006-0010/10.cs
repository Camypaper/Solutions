            ...
            dfs = (pos, val) =>
                {
                    ...
                    if (find) return true;
                    if (pos == n - 1)
                        return (dp[pos, val] = find = val == total).Value;
                    if (dp[pos, val].HasValue) return dp[pos, val].Value;
                    o[pos] = '+';
                    if (dfs(pos + 1, val + a[pos + 1]))
                        return (dp[pos, val] = true).Value;
                    o[pos] = '*';
                    if (dfs(pos + 1, val * a[pos + 1]))
                        return (dp[pos, val] = true).Value;
                    return (dp[pos, val] = false).Value;
                };
            dfs(0, a[0]);
            IO.Printer.Out.WriteLine(o.AsString());
        }
