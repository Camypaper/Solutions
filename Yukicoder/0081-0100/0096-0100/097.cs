    ...
    public class Solver
    {
        public void Solve()
        {
            ...
            foreach (var x in sc.Integer(q))
            {
                ...
                var inv = MathEx.Inverse(x, mod);
                var pass = false;
                if (n > 1000)
                {
                    for (int i = 100002; i >= 90000; i--)
                        if (set.Contains((i * inv) % mod))
                        {
...
{
    //use when num and mod are coprime!
    static public long Inverse(long num, long mod)
    {
